# main.py - GPU优化完整修复版（修复pin_memory问题）
import os
import numpy as np
import torch
...
class FPACOACNNTrainer:
    ...
    def __init__(self, config):
        ...
        self.best_model_path = "model/best_fpa_coa_cnn.pth"
        
        # GPU优化设置
        self.gpu_trainer = GPUTrainer(config) if self.use_gpu else None
    ...
    def create_feature_selection_objective(self, X_train, y_train, X_val, y_val):
        ...
        def objective(feature_subset):
            ...
            # 使用快速评估配置
            eval_batch_size = self.config.FEATURE_SELECTION.get('eval_batch_size', 128)
            
            # 修复：对于CPU上的Tensor，可以启用pin_memory加速GPU传输
            # 关键：数据在CPU上时，pin_memory=True可以加速
            # 数据在GPU上时，pin_memory=False（因为不能pin GPU内存）
            pin_memory = self.use_gpu  # 如果使用GPU，启用pin_memory加速
            
            train_loader = DataLoader(
                train_dataset, 
                batch_size=eval_batch_size, 
                shuffle=True,
                pin_memory=pin_memory,  # 重要修复：根据情况启用
            ...
            # 快速训练
            eval_epochs = self.config.FEATURE_SELECTION.get('eval_epochs', 3)
            model.train()
    ...
    def create_safe_dataloader(self, dataset, batch_size=128, shuffle=True, for_training=True):
        """创建安全的DataLoader，正确处理pin_memory"""
        
        # 决定是否使用pin_memory
        # 规则：只有在以下情况下才启用pin_memory：
        # 1. 使用GPU
        # 2. 数据在CPU上
        # 3. 是训练过程（而不是快速评估）
        
        if for_training and self.use_gpu:
            # 对于训练过程，启用pin_memory加速
            pin_memory = True
            num_workers = min(4, os.cpu_count() // 2) if self.use_gpu else 0
        else:
            # 对于快速评估或CPU训练，禁用pin_memory
            pin_memory = False
            num_workers = 0
    ...
    def prepare_cnn_datasets(self, X_train, X_val, X_test, y_train, y_val, y_test, 
        ...
        train_loader = self.create_safe_dataloader(
            train_dataset, batch_size=batch_size, shuffle=True, for_training=True
        )
        
        val_loader = self.create_safe_dataloader(
            val_dataset, batch_size=batch_size, shuffle=False, for_training=False
        )
        
        test_loader = self.create_safe_dataloader(
            test_dataset, batch_size=batch_size, shuffle=False, for_training=False
        )
    ...
    def evaluate_model(self, X_test, y_test, label_names=None):
        ...
        # 重塑为CNN格式
        sequence_length = 100
        X_test_cnn = reshape_for_cnn(X_test_selected, sequence_length)
    ...
    def run(self):
        """运行完整的FPA-COA-CNN流程"""
        print("\n" + "="*60)
        print("FPA-COA-CNN入侵检测系统（GPU优化版）")
        print("="*60)
